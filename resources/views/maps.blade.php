      ...
      <header class="py-0 md:py-1">
         <div class="px-5">
            <div class="gap-5 flex max-md:flex-col max-md:items-stretch max-md:gap-0">
               ...
               <div class="flex flex-col items-stretch w-[47%] ml-5 max-md:w-full max-md:ml-0 border-none">
                  <div id="searchss" class="bg-gray-200 flex items-center justify-between gap-5 w-full my-auto pl-5 pr-2 py-2 rounded-xl max-md:max-w-full max-md:flex-wrap max-md:mt-10">
                     <div id="searchIcon"class="text-indigo-500 text-lg font-medium grow whitespace-nowrap my-auto w-full bg-transparent border-none outline-none">
                        {{-- <input id="searchInput" type="text" class="text-indigo-500 text-lg font-medium grow whitespace-nowrap my-auto w-full bg-transparent outline-none" placeholder="Search" /> --}}
                     </div>
                     {{-- 
         ...
         $(document).ready(function () {
             ...
             $.getJSON('/titik/json', function (data) {
                 const markers = data.map((item) => {
                     ...
                     html += '<h5 class="mb-1 text-lg font-semibold text-white">Nama Lokasi: ' + item.nama + ' </h5>';
                     html += '<h5 class="mb-2 text-sm text-indigo-200">Alamat: ' + item.alamat + ' </h5>';
                     html += '<h5 class="mb-2 text-sm text-blue-200">Pertamax: ' + item.pertamax + ' </h5>';
                     html += '<h5 class="mb-2 text-sm text-green-200">Pertalite: ' + item.pertalite + ' </h5>';
                     html += '<h5 class="mb-2 text-sm text-yellow-200">Pertamax Turbo: ' + item.pertamax_turbo + ' </h5>';
                     html += '<h5 class="mb-2 text-sm text-red-200">Solar: ' + item.solar + ' </h5>';
                     html += '<img class="max-w-full mt-2 rounded-md" src="images/' + item.image + '">';
                     ...
                     // Create a marker and add it to the markerLayer
                     const marker = L.marker([parseFloat(item.latitude), parseFloat(item.longitude)], {
                         icon: gasIcon,
                         title: item.nama
                     }).addTo(markerLayer).bindPopup(html);
             ...
             // Add an event listener to the search input in the header
             ...
             // Optionally, fit the map bounds to the added markerLayer
